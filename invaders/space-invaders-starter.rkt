...
(check-expect (handle-key G0 " ")
              (make-game empty (list (make-missile (tank-x T0) TANK-HEIGHT/2)) T0))
(check-expect (handle-key G0 "left")
              (make-game '() '() (make-tank (tank-x T0) -1)))
(check-expect (handle-key G0 "left")
              (make-game '() '() (make-tank (tank-x T0) 1)))
(check-expect (handle-key G0 "up")
              (make-game '() '() T0))
; (define G3 (make-game (list I1 I2) (list M1 M2) T1))
(check-expect (handle-key G3 " ")
              (make-game (list I1 I2) (list (make-missile (tank-x T1) TANK-HEIGHT/2) M1 M2) T1))

(define (handle-key ws ke)
  (cond [(key=? ke " ") (make-game
                         game-invaders
                         (cons (make-missile (tank-x (game-tank ws)) TANK-HEIGHT/2) game-missiles)
                         game-tank)]
        [(key=? ke "left") (make-game
                         game-invaders
                         game-missiles
                         (make-tank (tank-x (game-tank ws)) -1))]
